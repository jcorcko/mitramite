...
begin
  ...
  if DateEdit1.Date>now then errores := errores + #149' La fecha es incorrecta.';
  Text1.Text := errores;
  if Text1.Text<>'' then
    ...
    begin
      try
        ...
        on E: EFDDBEngineException do begin
          ...
          begin
            Text1.Text := 'El número de Item ya fue introducido en un registro anterior.';
            RectMessage.Opacity := 0;
            RectMessage.AnimateFloat('Opacity', 1.0, 0.5, TAnimationType.In, TInterpolationType.Linear)
          end;
          mitramitedm.items.Cancel;
          abort;
        end;
      end;
      ModalResult := mrOK;
...
procedure TFitemedit.FormKeyUp(Sender: TObject; var Key: Word;
  var KeyChar: Char; Shift: TShiftState);
begin
  modificado := True;
end;

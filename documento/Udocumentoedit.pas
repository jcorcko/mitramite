...
type
  TFdocumentoedit = class(TForm)
    ...
    procedure Edit2KeyDown(Sender: TObject; var Key: Word; var KeyChar: Char;
      Shift: TShiftState);
    procedure Grid12CellClick(const Column: TColumn; const Row: Integer);
    procedure ComboTrackBar1ChangeTracking(Sender: TObject);
  private
...
var
  Fdocumentoedit: TFdocumentoedit;
...
procedure TFdocumentoedit.Button1Click(Sender: TObject);
var errores: String;
...
begin
  ...
  begin
    ...
    if mitramitedm.documentos.Locate('id', mitramitedm.qListarDocumentos.FieldByName('id').AsInteger, []) and modificado then
    begin
      try
        mitramitedm.documentos.Edit;
        ...
        mitramitedm.documentos.FieldByName('funcionario_id').AsString := mitramitedm.idfuncionario;
...
procedure TFdocumentoedit.ComboTrackBar1ChangeTracking(Sender: TObject);
begin
  modificado := True;
end;

procedure TFdocumentoedit.Edit2ChangeTracking(Sender: TObject);
begin
...
procedure TFdocumentoedit.Edit2KeyDown(Sender: TObject; var Key: Word;
...
procedure TFdocumentoedit.Edit3ChangeTracking(Sender: TObject);
begin
  mitramitedm.qListarTramites.Filter := 'tramite like ''%'+ Edit3.Text + '%''';
end;

procedure TFdocumentoedit.EditButton1Click(Sender: TObject);
begin
...
procedure TFdocumentoedit.FormKeyUp(Sender: TObject; var Key: Word;
  var KeyChar: Char; Shift: TShiftState);
begin
...
procedure TFdocumentoedit.Grid12CellClick(const Column: TColumn;
  const Row: Integer);
begin
  if Column.Index>0 then modificado := True;
end;

procedure TFdocumentoedit.SpeedButton12Click(Sender: TObject);
begin
...
procedure TFdocumentoedit.SpeedButton2Click(Sender: TObject);
begin
  Close;
end;

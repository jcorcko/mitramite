...
procedure TFcargoedit.Button1Click(Sender: TObject);
var errores: String;
begin
  errores := '';
  ...
  begin
    ...
    if mitramitedm.cargos.Locate('id', mitramitedm.qListarCargos.FieldByName('id').AsInteger, []) and modificado then
    begin
...
procedure TFcargoedit.Edit1KeyDown(Sender: TObject; var Key: Word;
  var KeyChar: Char; Shift: TShiftState);
begin
  if (Key in [VKUP, VKDOWN, VKLEFT, VKRIGHT, VKBACK, VKDELETE, VKTAB])or(KeyChar in ['0'..'9']) then else Abort;
end;

procedure TFcargoedit.FormKeyUp(Sender: TObject; var Key: Word;
  var KeyChar: Char; Shift: TShiftState);
begin
  if (KeyChar in ['a'..'z', 'A'..'Z', '0'..'9']) or (Key = VKDELETE) then modificado := True;

end;

procedure TFcargoedit.SpeedButton12Click(Sender: TObject);
begin
...
procedure TFcargoedit.SpeedButton2Click(Sender: TObject);
begin
  Close;
end;
